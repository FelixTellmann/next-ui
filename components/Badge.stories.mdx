import { Meta, Story, ArgsTable, Canvas, Source } from '@storybook/addon-docs/blocks';
import { Badge }  from './Badge';
import { linkTo } from '@storybook/addon-links';


<Meta title="FelixTellmann.com/Badge" component={Badge} />


export const Template = (args) => <Badge {...args} />;

# Badge

Let's define a story for our `Badge` component:

We can drop it in a `Canvas` to get a code snippet:

<Canvas withToolbar>
  <Story name="positive" >
    {(args) => <Badge status="positive" onClick={linkTo('Heading','Heading')}{...args}>positive</Badge>}
  </Story>
</Canvas>

<ArgsTable story="positive" />

We can even preview multiple stories in a block. This
gets rendered as a group, but defines individual stories
with unique URLs and isolated snapshot tests.

<Canvas>
  <Story name="negative">
    <Badge status="negative">negative</Badge>
  </Story>
  <Story name="negative">
    <Badge status="negative">Button</Badge>
  </Story>
  <Story name="warning">
    <Badge status="warning">Warning</Badge>
  </Story>
  <Story name="neutral">
    <Badge status="neutral">neutral</Badge>
  </Story>
  <Story name="error">
    <Badge status="error">error</Badge>
  </Story>
</Canvas>

